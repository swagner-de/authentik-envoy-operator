...
// +kubebuilder:rbac:groups=authentik-envoy-operator.io,resources=authentikproviders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=authentik-envoy-operator.io,resources=authentikproviders/status,verbs=get;update;patch
// +kubebuilder:rbac:groups=authentik-envoy-operator.io,resources=authentikproviders/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AuthentikProvider object against the actual cluster state, and then
...
func (r *AuthentikProviderReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&authentikenvoyoperatoriov1alpha1.AuthentikProvider{}).
		Named("authentikprovider").
		Complete(r)
}
